...
def pad_collate_fn(batch, padding_value):
    ...
    perm_idx = sorted(range(len(lengths)),
                      key=lambda k: lengths[k], reverse=True)
    ...
    return paded_sentences, topics, lengths
...
if __name__ == '__main__':
    ...
    for i, (sent, topic, lengths) in enumerate(data_loader):
        # print(sent.size)
        print(sent)
        print(lengths)
        print(topic)
