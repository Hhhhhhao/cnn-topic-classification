...
class Trainer(object):
    ...
    def build_model(self):
        ...
        if self.config.pretrain_emb:
            ...
            weight_matrix = self.vocabulary.get_word2vec_weight(
                model_name='glove.42B', emb_size=self.config.emb_size)
            weight_tensor = torch.FloatTensor(weight_matrix)
            # self.model.init_emb(weight_tensor, freeze=self.config.static)

        self.model = TopicClassLSTM(
            vocab_size=self.config.vocab_size, 
            emb_size=self.config.emb_size,
            embedding_tensor=weight_tensor,
    ...
    def train_epoch(self, epoch):
        self.model.train()
        for batch_idx, (sentences, gt_topics, lengths) in enumerate(self.train_loader):
            sentences = sentences.to(self.device)
            gt_topics = gt_topics.to(self.device)

            pred_topics = self.model(sentences, lengths)
    ...
    def valid_epoch(self, epoch):
        ...
        val_acc5 = []
        with torch.no_grad():
            for batch_idx, (sentences, gt_topics, lengths) in enumerate(self.valid_loader):
